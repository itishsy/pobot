...
def compare_players(player1, player2, community_cards):
    # 获取每个玩家的最佳手牌分数和详细信息
    player1_score, player1_details = get_best_hand(player1.hand, community_cards)
    player2_score, player2_details = get_best_hand(player2.hand, community_cards)
...
def distribute_pot(active_players, community_cards, pot):
    # 1. 处理只有一个活跃玩家的情况
    if len(active_players) == 1:
        active_players[0].chips += pot
        return {'winner': active_players[0], 'split_pot': False, 'message': f"{active_players[0].name} 获胜！赢得 {pot} 筹码"}

    # 2. 筛选出所有-in的玩家和他们的all-in金额
    all_in_players = [p for p in active_players if p.all_in]
    if not all_in_players:
        # 没有all-in玩家，按照常规方式分配底池
        return distribute_regular_pot(active_players, community_cards, pot)

    # 3. 计算每个玩家在这一轮的总投入（包括之前的下注）
    for player in active_players:
        # 假设player对象有一个bet_this_round属性记录本轮下注
        # 如果没有，我们可以在这里初始化它为0
    ...
    for bet in unique_bets[1:]:  # 跳过最小的下注
        # 计算这个边池的大小
    ...
    for pot_size, eligible_players in side_pots:
        ...
        for player in eligible_players[1:]:
            result = compare_players(best_player, player, community_cards)
            if result < 0:
                best_player = player
                tied_players = [best_player]
            elif result == 0:
        ...
        if len(tied_players) > 1:
            ...
            messages.append(f"边池 {pot_size} 由 {', '.join([p.name for p in tied_players])} 平分")

            for i, player in enumerate(tied_players):
                player.chips += split_amount
                if i < remainder:
                    player.chips += 1
                    messages.append(f"{player.name} 获得 {split_amount + 1} 筹码")
                else:
                    messages.append(f"{player.name} 获得 {split_amount} 筹码")
            ...
            messages.append(f"{best_player.name} 赢得边池 {pot_size}")
            results.append({
                'winner': best_player,
                'split_pot': False,
                'pot_size': pot_size
...
def distribute_regular_pot(active_players, community_cards, pot):
    # 比较所有玩家的牌型
    ...
    if len(tied_players) > 1:
        ...
        message = f"底池 {pot} 由 {', '.join([p.name for p in tied_players])} 平分\n"

        for i, player in enumerate(tied_players):
            player.chips += split_amount
            if i < remainder:
                player.chips += 1
                message += f"{player.name} 获得 {split_amount + 1} 筹码\n"
            else:
                message += f"{player.name} 获得 {split_amount} 筹码\n"

        return {'winner': None, 'split_pot': True, 'tied_players': tied_players, 'message': message}
    else:
        # 单一获胜者
        best_player.chips += pot
        return {'winner': best_player, 'split_pot': False, 'message': f"{best_player.name} 获胜！赢得 {pot} 筹码"}